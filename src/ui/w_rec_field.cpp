#include "widget/utils/w_rec_field.h"
...
void W_Rec_Field::refresh(ETable _rec_table)
{
  rec_table = _rec_table;
}

void W_Rec_Field::set_record(ETable _rec_table, qsizetype _id)
{
  rec_table = _rec_table;
  id        = _id;
...
{
  assert(rec_table != ETable::NONE && "Invalid record table");
